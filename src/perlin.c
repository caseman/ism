 ...
 */
unsigned char default_perm[] = {151,160,137,91,90,15,
...
/*
 * Generate new randomized permutation table
 */
noise_ptable create_noise_ptable(unsigned int (*rand_func)())
{
    noise_ptable ptable = malloc(sizeof(default_perm));
    if (ptable == NULL) {
        return NULL;
    }
    memcpy(ptable, default_perm, sizeof(default_perm));
    for (int i = 0; i < 256; i++) {
        unsigned int r = rand_func();
        unsigned char swap_i = (r^(r>>8)^(r>>16)^(r>>24)) & 0xf;
        unsigned char t = ptable[i];
        ptable[i] = ptable[swap_i];
        ptable[i + 255] = ptable[swap_i];
        ptable[swap_i] = t;
        ptable[swap_i + 255] = t;
    }
    return ptable;
 ...
 * float SLnoise = (noise3(noise_ptable perm,x,y,z) + 1.0) * 0.5;
 */
 ...
 */
float noise1(noise_ptable perm, float x)
{
    int ix0, ix1;
    float fx0, fx1;
    float s, n0, n1;
 ...
 */
float pnoise1(noise_ptable perm, float x, int px)
{
    int ix0, ix1;
    float fx0, fx1;
    float s, n0, n1;
 ...
 */
float noise2(noise_ptable perm, float x, float y)
{
    int ix0, iy0, ix1, iy1;
 ...
 */
float pnoise2(noise_ptable perm, float x, float y, int px, int py)
{
    int ix0, iy0, ix1, iy1;
    float fx0, fy0, fx1, fy1;
    float s, t, nx0, nx1, n0, n1;
 ...
 */
float noise3(noise_ptable perm, float x, float y, float z)
{
 ...
 */
float pnoise3(noise_ptable perm, float x, float y, float z, int px, int py, int pz)
{
    int ix0, iy0, ix1, iy1, iz0, iz1;
    float fx0, fy0, fz0, fx1, fy1, fz1;
    float s, t, r;
...
float noise4(noise_ptable perm, float x, float y, float z, float w)
{
...
float pnoise4(noise_ptable perm, float x, float y, float z, float w,
